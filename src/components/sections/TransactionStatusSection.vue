...
const emit = defineEmits<{
  ...
  'update:rreqWeightU': [value: string]
  'update:rreqWeightR': [value: string]
...
const getRreqWeight = (key: string): number => {
  switch (key) {
    ...
    case 'rreqWeightU':
      return Number(props.rreqWeightU) || 0
    case 'rreqWeightR':
      return Number(props.rreqWeightR) || 0
    default:
      return 0
  }
}
...
const onRreqWeightInput = (key: string, event: Event) => {
  ...
  switch (key) {
    ...
    case 'rreqWeightN':
      ...
      break
    case 'rreqWeightU':
      emit('update:rreqWeightU', String(nextValue))
      break
    case 'rreqWeightR':
      emit('update:rreqWeightR', String(nextValue))
      break
  }
}
      ...
      <div class="mt-4 rounded-md bg-base-200 p-4">
        ...
        <div class="mt-4 border-t border-base-300 pt-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="md:row-start-3 md:col-start-1">
              <label class="label">
                <span class="label-text">
                  RReq U 權重% (rreq_transStatus=U)
                  <span class="text-xs text-base-content/60 ml-2"> {{ props.rreqWeightU }}% </span>
                </span>
              </label>
              <input
                id="rreqWeightU"
                ...
                max="100"
                step="1"
                :value="props.rreqWeightU"
                @input="(event) => onRreqWeightInput('rreqWeightU', event)"
              />
            </div>
            <div class="md:row-start-4 md:col-start-1">
              <label class="label">
                <span class="label-text">
                  RReq R 權重% (rreq_transStatus=R)
                  <span class="text-xs text-base-content/60 ml-2"> {{ props.rreqWeightR }}% </span>
                </span>
              </label>
              <input
                id="rreqWeightR"
                ...
                max="100"
                step="1"
                :value="props.rreqWeightR"
                @input="(event) => onRreqWeightInput('rreqWeightR', event)"
              />
            </div>
            <div class="md:row-start-5 md:col-start-1">
              <label class="label">
                <span class="label-text">
        ...
        <div class="mt-4 border-t border-base-300 pt-4">
          ...
          <p class="text-xs text-base-content/60 mb-3 leading-relaxed">
            <strong>transStatusReason</strong>：最終狀態為 <strong>R</strong> 時假資料會抽 ACS
            原因碼； <strong>ARes=C 且 RReq=N</strong>：ChallengeCancel 觸發率預設約 8%（Edit 8
            放棄率 ≤10%）； 觸發時偏重 <code class="bg-base-300/60 px-1 rounded">04</code>／
            <code class="bg-base-300/60 px-1 rounded">05</code> 並固定帶 reason
            <code class="bg-base-300/60 px-1 rounded">14</code>（D-02 拆
            <code class="bg-base-300/60 px-1 rounded">14_1005</code>／
            <code class="bg-base-300/60 px-1 rounded">14_1004</code>）；其他 cancel 碼可灌純
            <code class="bg-base-300/60 px-1 rounded">14</code>；cancel 為空則不灌
            14。寫入前會依索引對齊兩端後端： <strong>3DSS</strong>（<code
              class="bg-base-300/60 px-1 rounded"
              >3dss-transaction</code
