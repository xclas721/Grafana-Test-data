import {
  ...
  getTransStatusReasonCandidates,
  isAresFailureStatus,
  isRreqFailureStatus
...
/** Challenge 取消碼：偏重 04／05，供 D-02 reason 14 拆桶（14_1005／14_1004）。 */
const CHALLENGE_CANCEL_OTHER = ['01', '02', '03', '06', '07', '09', '10'] as const
/** cancel 已寫入但非 04／05 時，灌 reason=14 的比例（D-02 純 14 其餘桶）。 */
const REASON_14_WHEN_CANCEL_OTHER_RATE = 0.85

function pickChallengeCancelForTimeoutMix(random: () => number): string {
  const r = random()
  // 約 80% 落在 04／05，其餘給其他 cancel（純 14 桶）
  if (r < 0.4) return '04'
  if (r < 0.8) return '05'
  return pickRandom(CHALLENGE_CANCEL_OTHER, random)
}
...
export function randomizeBusinessFields(
  ...
  const aresFail = isAresFailureStatus(input.aresTransStatus)
  const rreqFail = isRreqFailureStatus(input.rreqTransStatus)
  const challengeFail = (input.aresTransStatus === 'C' || input.aresTransStatus === 'D') && rreqFail

  if (aresFail) {
    // Insight B-11：ARes N/U/R 都要有 transStatusReason，熱力才會出現三欄
  ...
  } else if (challengeFail) {
    // D-02／B-12：挑戰失敗（RReq N/U/R）可灌 reason 14 + challengeCancel
    const shouldSetCancel = random() < input.challengeCancelRate
    const challengeCancel = shouldSetCancel
      ? pickChallengeCancelForTimeoutMix(random)
      : 'NULL_VALUE'
    updates.challengeCancel = challengeCancel

    if (challengeCancel === '04' || challengeCancel === '05') {
      updates.transStatusReason = '14'
      // ACS D-02 拆 14_1004／14_1005 看 stateMachineReason；3DSS 看 challengeCancel
      const linkedStm = acsStateMachineReasonForChallengeCancel(challengeCancel)
      if (input.activeMode !== 'dss' && linkedStm) {
        updates.stateMachineReason = linkedStm
      }
    } else if (challengeCancel !== 'NULL_VALUE' && random() < REASON_14_WHEN_CANCEL_OTHER_RATE) {
      updates.transStatusReason = '14'
    } else if (input.transStatusReasonMode === 'random') {
      // 非 14 路徑：灌一般 reason（勿灌 14 卻缺 cancel）；供 B-12 熱力 N/U/R 有資料
      const candidates = getTransStatusReasonCandidates(effectiveCardScheme).filter(
        (c) => c !== '14'
      )
      updates.transStatusReason =
        candidates.length > 0
