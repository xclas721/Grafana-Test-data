...
function loadDefaults() {
  ...
  config.requestCount = 20
  config.acctNumber = '4143520000000124'
...
async function runTest() {
  ...
  addLog('info', '=== CReq Checkpoint2 Rate Limit Test ===')
  ...
  try {
    ...
    addLog('info', 'Expected: observe transition from PASS to BLOCKED')
    addLog('info', '')
  } catch (e: unknown) {
  ...
  if (stats.successCount >= 1 && stats.rateLimitCount >= 1 && stats.otherErrorCount === 0) {
    addLog('success', 'Result: PASS (observed transition from PASS to BLOCKED)')
  } else {
  ...
  <Card :title="CARD_TITLES.config" subtitle="CReq 檢查點2 - 觀察由通過轉為限流">
    <div class="space-y-4">
      ...
      <DDoSSection :title="SECTION_TITLES.request">
        <Input v-model="config.acctNumber" :label="LABELS.acctNumber" placeholder="4143520000000124" :disabled="isTesting" />
