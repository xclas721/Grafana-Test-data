...
async function runTest() {
  ...
  addLog('info', `=== AReq Card Rate Limit Test ===`)
  addLog('info', `URL: ${url}`)
  addLog('info', `CardCount: ${config.cardCount}, RequestsPerCard: ${config.requestsPerCard}`)
  addLog('info', `Expected: first 5 per card pass, 6th+ blocked`)
  addLog('info', '')

  const totalRequests = config.cardCount * config.requestsPerCard
  let currentIndex = 0

  for (let cardIdx = 1; cardIdx <= config.cardCount; cardIdx++) {
    if (shouldStop.value) break
    const cardNumber = config.cardPrefix + (120 + cardIdx).toString().padStart(4, '0')
    addLog('info', `--- Card ${cardIdx}/${config.cardCount} : ${cardNumber} ---`)

    for (let reqIdx = 1; reqIdx <= config.requestsPerCard; reqIdx++) {
      if (shouldStop.value) break
      currentIndex++
      ...
      try {
        ...
        if (data?.messageType === 'ARes') {
          stats.successCount++
          addLog('success', `  [${currentIndex}/${totalRequests}] PASS (${ms}ms)`)
  ...
  const expectedBlocked = config.cardCount * Math.max(0, config.requestsPerCard - 5)
  addLog('info', `=== Summary ===`)
  addLog('info', `PASS: ${stats.successCount} (expected: ${config.cardCount * 5})`)
...
const expectedBlocked = computed(() => config.cardCount * Math.max(0, config.requestsPerCard - 5))
</script>
...
<template>
  ...
  <DDoSTestIntro test-type="areqCard" class="mb-6" />
